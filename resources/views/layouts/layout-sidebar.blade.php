<nav class="sidebar sidebar-offcanvas bg-dark"  id="sidebar">
    <div class="user-profile">
        <div class="user-image">
            <img src="data:image/png;base64,">
        </div>
        <div class="user-name">
            {{Auth::user()->name}}
        </div>
        <div class="user-designation fs9">
            {{-- {{$authData['jabatan']}} --}}
            Manager
        </div>
    </div>
    
    
    <ul class="nav">
        {{-- Dashboard --}}
        <li class="nav-item @if($checkPoint == 'dashboard') active @endif">
            <a class="nav-link" href="{{url('dashboard')}}" style="border:none">
                <i class="fas fa-box menu-icon" style="font-size:12px;"></i>
                <span class="menu-title " style="font-size:12px;">Dashboard</span>
            </a>
        </li>
        <li class="nav-item @if($checkPoint == 'report') active @endif">
            <a class="nav-link" data-toggle="collapse" href="#customer" style="border:none" aria-expanded="false" aria-controls="customer">
                <i class="fas fa-copy menu-icon" style="font-size:12px;"></i>
                <span class="menu-title " style="font-size:12px;">Report</span>
                <i class="menu-arrow"></i>
            </a>
            <div class="collapse" id="customer">
                <ul class="nav flex-column sub-menu">
                <li class="nav-item"> <a class="nav-link text-white" style="font-size:10px; border:none;" href="{{url('data-pesanan-kalibrasi')}}">Data Pesanan</a></li>
                <li class="nav-item"> <a class="nav-link text-white" style="font-size:10px; border:none;" href="{{url('data-pelanggan')}}">Data Pelanggan</a></li>
                </ul>
            </div>
        </li>
        <li class="nav-item @if($checkPoint == 'manage_data') active @endif">
            <a class="nav-link" data-toggle="collapse" href="#manage_data" style="border:none" aria-expanded="false" aria-controls="manage_data">
                <i class="fas fa-copy menu-icon" style="font-size:12px;"></i>
                <span class="menu-title " style="font-size:12px;">Manage Data</span>
                <i class="menu-arrow"></i>
            </a>
            <div class="collapse" id="manage_data">
                <ul class="nav flex-column sub-menu">
                <li class="nav-item"> <a class="nav-link text-white" style="font-size:10px; border:none;" href="{{url('tambah-pesanan')}}">Tambah Pesanan</a></li>
                <li class="nav-item"> <a class="nav-link text-white" style="font-size:10px; border:none;" href="">Tambah Pelanggan</a></li>
                </ul>
            </div>
        </li>
        {{-- Customer --}}
        {{-- <li class="nav-item @if($checkPoint == 'customer') active @endif">
            <a class="nav-link" data-toggle="collapse" href="#customer" style="border:none" aria-expanded="false" aria-controls="customer">
                <i class="fas fa-user menu-icon" style="font-size:12px;"></i>
                <span class="menu-title " style="font-size:12px;">Customer</span>
                <i class="menu-arrow"></i>
            </a>
            <div class="collapse" id="customer">
                <ul class="nav flex-column sub-menu">
                <li class="nav-item"> <a class="nav-link text-white" style="font-size:10px; border:none;" href="{{route('addCustomer')}}">Add New Customer</a></li>
                <li class="nav-item"> <a class="nav-link text-white" style="font-size:10px; border:none;" href="{{route('customerData')}}">Customer Data</a></li>
                </ul>
            </div>
        </li> --}}
        {{-- Pre Order --}}
        {{-- <li class="nav-item @if($checkPoint == 'proforma_master') active @endif">
            <a class="nav-link" href="{{url('pre_order')}}" style="border:none">
                <i class="fas fa-paste menu-icon" style="font-size:12px;"></i>
                <span class="menu-title " style="font-size:12px;">PO Master</span>
            </a>
        </li>
        <li class="nav-item @if($checkPoint == 'house_po') active @endif">
            <a class="nav-link" href="{{url('p_house')}}" style="border:none">
                <i class="fas fa-paperclip menu-icon" style="font-size:12px;"></i>
                <span class="menu-title" style="font-size:12px;">House PO</span>
            </a>
        </li> --}}
        {{-- <li class="nav-item @if($checkPoint == 'shipping_instruction') active @endif">
            <a class="nav-link" href="{{url('shipping_instruction')}}" style="border:none">
                <i class="fas fa-check menu-icon" style="font-size:12px;"></i>
                <span class="menu-title" style="font-size:12px;">Shipping Instruction</span>
            </a>
        </li> --}}
        {{-- <li class="nav-item @if($checkPoint == 'invoice') active @endif">
            <a class="nav-link" href="{{url('make_invoice')}}" style="border:none">
                <i class="fas fa-bullhorn menu-icon" style="font-size:12px;"></i>
                <span class="menu-title " style="font-size:12px;">Make Invoice</span>
            </a>
        </li> --}}
    </ul>
</nav>